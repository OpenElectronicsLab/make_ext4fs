...
static u8 *create_backing(struct fs_info *info,
...
static void reserve_indirect_block(int force, jmp_buf *setjmp_env,
				   struct block_allocation *alloc, int len)
{
	if (reserve_oob_blocks(alloc, setjmp_env, 1)) {
		error(force, setjmp_env, "failed to reserve oob block");
		return;
	}

	if (advance_blocks(alloc, len)) {
		error(force, setjmp_env, "failed to advance %d blocks", len);
		return;
	}
}

static void reserve_dindirect_block(struct fs_aux_info *aux_info, int force,
				    jmp_buf *setjmp_env,
				    struct block_allocation *alloc, int len)
{
	if (reserve_oob_blocks(alloc, setjmp_env, 1)) {
	...
	while (len > 0) {
		...
		reserve_indirect_block(force, setjmp_env, alloc, ind_block_len);
...
static void reserve_tindirect_block(struct fs_aux_info *aux_info, int force,
				    jmp_buf *setjmp_env,
				    struct block_allocation *alloc, int len)
{
	if (reserve_oob_blocks(alloc, setjmp_env, 1)) {
	...
	while (len > 0) {
		...
		reserve_dindirect_block(aux_info, force, setjmp_env, alloc,
					dind_block_len);
...
static void fill_dindirect_block(struct fs_info *info,
...
static void fill_tindirect_block(struct fs_info *info,
				 struct fs_aux_info *aux_info,
				 struct sparse_file *ext4_sparse_file,
				 int force, jmp_buf *setjmp_env,
				 u32 *tind_block, int len,
				 struct block_allocation *alloc)
{
	int i;
	u32 dind_block;
	...
	for (i = 0; len > 0; i++) {
		...
		fill_dindirect_block(info, aux_info, ext4_sparse_file, force,
				     setjmp_env, dind_block_data,
				     dind_block_len, alloc);
...
static int inode_attach_direct_blocks(int force, jmp_buf *setjmp_env,
...
static int inode_attach_indirect_blocks(struct fs_info *info,
					struct fs_aux_info *aux_info,
					struct sparse_file *ext4_sparse_file,
					int force, jmp_buf *setjmp_env,
					struct ext4_inode *inode,
					...
					u32 *block_len)
{
	...
	if (advance_oob_blocks(alloc, 1)) {
		error(force, setjmp_env, "failed to advance oob block");
		return -1;
	}
...
static int inode_attach_dindirect_blocks(struct fs_info *info,
					 struct fs_aux_info *aux_info,
					 struct sparse_file *ext4_sparse_file,
					 int force, jmp_buf *setjmp_env,
					 struct ext4_inode *inode,
...
static int inode_attach_tindirect_blocks(struct fs_info *info,
					 struct fs_aux_info *aux_info,
					 struct sparse_file *ext4_sparse_file,
					 int force, jmp_buf *setjmp_env,
					 struct ext4_inode *inode,
...
static void reserve_all_indirect_blocks(struct fs_aux_info *aux_info,
					int force, jmp_buf *setjmp_env,
					struct block_allocation *alloc, u32 len)
{
	...
	reserve_tindirect_block(aux_info, force, setjmp_env, alloc,
				tind_block_len);
	...
	error(force, setjmp_env, "%d blocks remaining", len);
}

static u32 indirect_blocks_needed(struct fs_aux_info *aux_info,
				  jmp_buf *setjmp_env, u32 len)
{
	...
	critical_error(setjmp_env, "request too large");
	return 0;
}

static int do_inode_attach_indirect(struct fs_info *info,
				    struct fs_aux_info *aux_info,
				    struct sparse_file *ext4_sparse_file,
				    int force, jmp_buf *setjmp_env,
				    struct ext4_inode *inode,
				    ...
				    u32 block_len)
{
	...
	if (count > 0) {
		if (inode_attach_indirect_blocks(info, aux_info,
						 ext4_sparse_file, force,
						 setjmp_env, inode, alloc,
						 &count)) {
			error(force, setjmp_env,
			      "failed to attach indirect blocks to inode");
			return -1;
		}
	}

	if (count > 0) {
		if (inode_attach_dindirect_blocks(info, aux_info,
						  ext4_sparse_file, force,
						  setjmp_env, inode, alloc,
						  &count)) {
			error(force, setjmp_env,
			      "failed to attach dindirect blocks to inode");
			return -1;
		}
	}

	if (count > 0) {
		if (inode_attach_tindirect_blocks(info, aux_info,
						  ext4_sparse_file, force,
						  setjmp_env, inode, alloc,
						  &count)) {
			error(force, setjmp_env,
			      "failed to attach tindirect blocks to inode");
			return -1;
		}
	}
	...
	if (count) {
		error(force, setjmp_env,
		      "blocks left after triply-indirect allocation");
		return -1;
	}
...
static struct block_allocation *do_inode_allocate_indirect(struct fs_aux_info
							   *aux_info, int force,
							   jmp_buf *setjmp_env,
							   u32 block_len)
{
	u32 indirect_len = indirect_blocks_needed(aux_info, setjmp_env,
						  block_len);

	struct block_allocation *alloc =
	    allocate_blocks(aux_info, force, setjmp_env,
			    block_len + indirect_len);

	if (alloc == NULL) {
		error(force, setjmp_env, "Failed to allocate %d blocks",
		      block_len + indirect_len);
		return NULL;
	}
			     ...
			     struct ext4_inode *inode, unsigned long len)
{
	...
	alloc = do_inode_allocate_indirect(aux_info, force, setjmp_env,
					   block_len);
	if (alloc == NULL) {
		error(force, setjmp_env,
		      "failed to allocate extents for %lu bytes", len);
		return;
	}

	reserve_all_indirect_blocks(aux_info, force, setjmp_env, alloc,
				    block_len);
	rewind_alloc(alloc);

	if (do_inode_attach_indirect(info, aux_info, ext4_sparse_file, force,
				     setjmp_env, inode, alloc, block_len))
		error(force, setjmp_env,
				 ...
				 unsigned long backing_len)
{
	...
	if (backing_len) {
		data = create_backing(info, ext4_sparse_file, setjmp_env, alloc,
				      backing_len);
		if (!data)
			error(force, setjmp_env,
			      "failed to create backing for %lu bytes",
			      backing_len);
	}
