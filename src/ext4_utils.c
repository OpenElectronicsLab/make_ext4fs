...
void read_sb(jmp_buf *setjmp_env, int fd, struct ext4_super_block *sb)
{
	...
	if (ret < 0)
		critical_error_errno(setjmp_env, "failed to seek to superblock");
			       ...
			       int force, jmp_buf *setjmp_env)
{
	...
	u8 *journal_data = inode_allocate_data_extents(info, aux_info,
				ext4_sparse_file, force, setjmp_env, inode,
				info->journal_blocks * info->block_size,
				info->journal_blocks * info->block_size);
	if (!journal_data) {
	error(force, setjmp_env, "failed to allocate extents for journal data");
		return;
	}
...
void ext4_update_free(struct fs_aux_info *aux_info)
{
	...
	for (i = 0; i < aux_info->groups; i++) {
		...
		aux_info->bg_desc[i].bg_used_dirs_count += get_directories(aux_info, i);
	     ...
	     jmp_buf *setjmp_env, int fd, int verbose)
{
	...
	if (ret < 0)
		critical_error_errno(setjmp_env, "failed to seek to end of input image");

	ret = lseek(fd, info->block_size * (aux_info->first_data_block + 1), SEEK_SET);
	if (ret < 0)
		critical_error_errno(setjmp_env, "failed to seek to block group descriptors");

	ret = read(fd, aux_info->bg_desc, info->block_size * aux_info->bg_desc_blocks);
	if (ret < 0)
		critical_error_errno(setjmp_env, "failed to read block group descriptors");
	if (ret != (int)info->block_size * (int)aux_info->bg_desc_blocks)
		critical_error(setjmp_env, "failed to read all of block group descriptors");
