					     ...
					     time_t fixed_time)
{
	...
	*dentries.inode = inode;
	inode_set_permissions(info, aux_info, ext4_sparse_file, force,
			      setjmp_env, inode, dentries.mode, dentries.uid,
			      dentries.gid, dentries.mtime);
	...
	for (i = 0; i < entries; i++) {
		if (dentries[i].file_type == EXT4_FT_REG_FILE) {
			...
			entry_inode = build_directory_structure(info, aux_info,
					ext4_sparse_file, force, setjmp_env,
...
int make_ext4fs_internal(struct fs_info *info, struct fs_aux_info *aux_info,
			 ...
			 jmp_buf *setjmp_env, int uuid_user_specified, int fd,
			 ...
			 FILE* block_list_file)
{
	...
	inode_set_permissions(info, aux_info, ext4_sparse_file, force,
			      setjmp_env, root_inode_num, root_mode, 0, 0,
